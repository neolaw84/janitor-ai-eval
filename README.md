...
## Architectural Design Pattern for Bots

Because the `janitor-ai-eval` scripts evaluate mathematically *before* the LLM generates its narrative text, the bot cannot anticipate the player's exact intent. The standard design pattern for advanced RPG bots is the **"LLM as Router, Script as Authority"** model.

*   **Script Authority:** The `scenario.md` file should contain separate pre-calculated JS blocks covering all possible actions based on the current state.
*   **LLM Routing:** The LLM reads the user's text, decides which "intent" fits best narratively, and binds its narration purely to the definitive `console.log()` instructions generated by the relevant JS block.
