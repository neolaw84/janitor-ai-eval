...
The parser accesses the active chat history via `context.chat.last_messages`, and extracts the bot's raw markdown text using the `.message` property.

It scans the LLM's response for key-value pairs formatted as:
`[key] [separator] [value]`
...
### Available APIs
- `Math`: `floor`, `ceil`, `round`, `max`, `min`, `abs`, `random`
...
- **Banned Syntax Tokens (Causes Exception)**: `function` definitions and ternary operators `? :` are NOT supported by the parser tree.
