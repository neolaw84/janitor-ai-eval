// patch-bundle.js
// Post-build script that runs after Webpack to remove Object.defineProperty calls from bundle.js
const fs = require('fs');
const path = require('path');

const bundlePath = path.resolve(__dirname, 'dist', 'bundle.js');
if (!fs.existsSync(bundlePath)) {
    console.error('bundle.js not found, run npm run build first');
    process.exit(1);
}
...
if (remaining > 0) {
    console.warn(`Warning: ${remaining} Object.defineProperty call(s) remain in bundle.js`);
} else {
    console.log('Success: No Object.defineProperty calls in bundle.js');
}

fs.writeFileSync(bundlePath, source, 'utf8');
console.log('bundle.js patched successfully.');
